...
export default function FinancierApplicationDetail() {
  ...
  const [documentTypes, setDocumentTypes] = useState({
    ...
    ulosottotodistus: { selected: false, required: false },
  });
  ...
  const handleCreateOffer = async () => {
    ...
    if (token?.startsWith('demo-token-')) {
      ...
      // RELEASE FINLAND DEMO: No admin - send directly to customer
      const newOffer = {
        id: Date.now(),
        application_id: parseInt(id),
        financier_id: 1,
        financier_name: 'Release Finland Oy',
        ...
        status: 'SENT', // Direct to customer - no admin approval needed
        notes_to_customer: offerData.notes_to_customer || '',
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
        // Include application info for display
      ...
      // Update application status to OFFER_SENT (customer can see it)
      if (application) {
  ...
  const handleSendDocumentRequest = async () => {
    ...
    if (token?.startsWith('demo-token-')) {
      ...
      const documentLabels: Record<string, string> = {
        ...
        henkilokortti: 'Virallinen henkilökortti / Passi',
        ulosottotodistus: 'Ulosottotodistus',
      };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Show contract creation when offer exists */}
      {offerList.length > 0 && !contractList.some(c => c.status !== 'DRAFT') && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="font-medium text-green-900">
                  {application.status === 'OFFER_ACCEPTED' ? 'Tarjous hyväksytty!' : 'Tarjous tehty - voit luoda sopimuksen'}
                </p>
                <p className="text-green-700 text-sm mt-1">
                  Asiakas: <strong>{application.contact_person || application.company_name}</strong>
                </p>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <button
                ...
                className="px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow-lg flex items-center text-lg"
              >
              ...
              >
                ...
                {showDocumentRequest ? 'Sulje' : 'Pyydä lisätietoja'}
              </button>
            </div>
          </div>
            ...
            >
              ...
              <div className="space-y-3 mb-4">
                {[
                  ...
                  { key: 'henkilokortti', label: 'Virallinen henkilökortti / Passi' },
                  { key: 'ulosottotodistus', label: 'Ulosottotodistus' },
                ].map(({ key, label }) => (
      ...
      {/* All documents received - Ready for contract */}
      {application.status === 'OFFER_ACCEPTED' && 
       infoRequestList.length > 0 && 
       infoRequestList.every(ir => ir.status === 'RESPONDED') && 
       !contractList.length && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="font-bold text-green-900 text-lg">Kaikki liitteet vastaanotettu!</p>
                <p className="text-green-700 mt-1">
                  Asiakas <strong>{application.contact_person || application.company_name}</strong> on toimittanut pyydetyt liitteet.
                </p>
                <p className="text-green-600 text-sm mt-1">
                  Voit nyt tehdä sopimuksen.
                </p>
              </div>
            </div>
            <button
              ...
              className="btn-primary bg-green-600 hover:bg-green-700 text-lg px-6 py-3"
            >
              <FileCheck className="w-5 h-5 mr-2" />
              Tee sopimus
            </button>
      ...
      <div className="border-b border-slate-200">
        <nav className="flex space-x-8">
          {(() => {
            // Check for unread customer responses
            const hasNewCustomerResponse = infoRequestList.some(
              (ir: any) => ir.sender === 'customer' && ir.status === 'RESPONDED'
            );
            ...
            return [
              ...
              { id: 'messages', label: 'Viestit', icon: MessageSquare, count: infoRequestList.length, hasNew: hasNewCustomerResponse },
            ].map((tab) => (
              <button
                key={tab.id}
              ...
              >
                ...
                {tab.count !== undefined && tab.count > 0 && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    tab.hasNew ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-600'
                  }`}>
                    {tab.count}
                  </span>
                )}
      ...
      <div>
              ...
              >
                ...
                <div className="bg-emerald-50 rounded-lg p-4 mb-6 border border-emerald-200">
                  ...
                  <div className="text-sm text-emerald-900 space-y-2">
                    <div className="grid grid-cols-2 gap-x-4">
                      <p>Kauppasumma: <strong>{formatCurrency(application.equipment_price)}</strong> <span className="text-emerald-600 text-xs">alv 0%</span></p>
                      <p className="text-slate-600">sis. alv: <strong>{formatCurrency(application.equipment_price * 1.255)}</strong></p>
                    </div>
                    <div className="grid grid-cols-2 gap-x-4">
                      <p>Käsiraha: <strong>{formatCurrency(parseFloat(offerData.upfront_payment || '0'))}</strong> <span className="text-emerald-600 text-xs">alv 0%</span></p>
                      <p className="text-slate-600">sis. alv: <strong>{formatCurrency(parseFloat(offerData.upfront_payment || '0') * 1.255)}</strong></p>
                    </div>
                    <div className="grid grid-cols-2 gap-x-4">
                      <p>Rahoitettava: <strong>{formatCurrency(application.equipment_price - parseFloat(offerData.upfront_payment || '0'))}</strong> <span className="text-emerald-600 text-xs">alv 0%</span></p>
                      <p className="text-slate-600">sis. alv: <strong>{formatCurrency((application.equipment_price - parseFloat(offerData.upfront_payment || '0')) * 1.255)}</strong></p>
                    </div>
                    <hr className="border-emerald-200 my-2" />
                    <div className="grid grid-cols-2 gap-x-4 bg-emerald-100 -mx-2 px-2 py-1 rounded">
                      <p className="text-emerald-900">Kk-maksu: <strong className="text-lg">{formatCurrency(parseFloat(offerData.monthly_payment || '0'))}</strong> <span className="text-emerald-600 text-xs">alv 0%</span></p>
                      <p className="text-slate-700">sis. alv: <strong className="text-lg">{formatCurrency(parseFloat(offerData.monthly_payment || '0') * 1.255)}</strong></p>
                    </div>
                    <div className="grid grid-cols-2 gap-x-4">
                    ...
                    <div className="grid grid-cols-2 gap-x-4">
                      <p>Avausmaksu: <strong>{formatCurrency(parseFloat(offerData.opening_fee || '300'))}</strong> <span className="text-emerald-600 text-xs">alv 0%</span></p>
                      <p className="text-slate-600">sis. alv: <strong>{formatCurrency(parseFloat(offerData.opening_fee || '300') * 1.255)}</strong></p>
                    </div>
                    <div className="grid grid-cols-2 gap-x-4">
                      <p>Laskutuslisä: <strong>{offerData.invoice_fee || '9'} €/kk</strong> <span className="text-emerald-600 text-xs">alv 0%</span></p>
                      <p className="text-slate-600">sis. alv: <strong>{formatCurrency(parseFloat(offerData.invoice_fee || '9') * 1.255)}</strong></p>
                    </div>
                    {offerData.residual_value && application.equipment_price && (
                      <div className="grid grid-cols-2 gap-x-4">
                        <p>Jäännösarvo: <strong>{((parseFloat(offerData.residual_value) / application.equipment_price) * 100).toFixed(1)}%</strong> ({formatCurrency(parseFloat(offerData.residual_value))})</p>
                        <p className="text-slate-600">sis. alv: <strong>{formatCurrency(parseFloat(offerData.residual_value) * 1.255)}</strong></p>
                      </div>
                    )}
                  </div>
                  <p className="text-xs text-emerald-700 mt-3 italic">ALV 25,5 %</p>
                </div>
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button onClick={handleCreateOffer} disabled={isSavingOffer} className="btn-primary">
                    <Send className="w-4 h-4 mr-2" />
                    {isSavingOffer ? 'Lähetetään...' : 'Lähetä tarjous asiakkaalle'}
                  </button>
                </div>
              </motion.div>
            )}
                  ...
                  <div className="bg-slate-50 rounded-xl p-5 mb-4">
                    <div className="space-y-2 text-sm">
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <span className="font-semibold text-midnight-900">{formatCurrency(application.equipment_price)} <span className="text-xs text-slate-500">alv 0%</span></span>
                        <span className="text-slate-600 text-right">{formatCurrency(application.equipment_price * 1.255)} <span className="text-xs">sis. alv</span></span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <span className="text-slate-600">Käsiraha:</span>
                        <span className="font-semibold text-midnight-900">{formatCurrency(offer.upfront_payment || 0)} <span className="text-xs text-slate-500">alv 0%</span></span>
                        <span className="text-slate-600 text-right">{formatCurrency((offer.upfront_payment || 0) * 1.255)} <span className="text-xs">sis. alv</span></span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <span className="text-slate-600">Rahoitettava:</span>
                        <span className="font-semibold text-midnight-900">{formatCurrency(application.equipment_price - (offer.upfront_payment || 0))} <span className="text-xs text-slate-500">alv 0%</span></span>
                        <span className="text-slate-600 text-right">{formatCurrency((application.equipment_price - (offer.upfront_payment || 0)) * 1.255)} <span className="text-xs">sis. alv</span></span>
                      </div>
                      <hr className="my-2 border-slate-200" />
                      <div className="grid grid-cols-3 gap-2 bg-emerald-50 -mx-2 px-2 py-2 rounded">
                        <span className="text-slate-700 font-medium">Kk-maksu:</span>
                        <span className="font-bold text-emerald-700 text-lg">{formatCurrency(offer.monthly_payment)} <span className="text-xs font-normal text-slate-500">alv 0%</span></span>
                        <span className="font-bold text-emerald-800 text-lg text-right">{formatCurrency(offer.monthly_payment * 1.255)} <span className="text-xs font-normal">sis. alv</span></span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <span className="text-slate-600">Sopimusaika:</span>
                        <span className="font-semibold text-midnight-900 col-span-2">{offer.term_months} kk</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <span className="text-slate-600">Avausmaksu:</span>
                        <span className="font-semibold text-midnight-900">300 € <span className="text-xs text-slate-500">alv 0%</span></span>
                        <span className="text-slate-600 text-right">{formatCurrency(300 * 1.255)} <span className="text-xs">sis. alv</span></span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <span className="text-slate-600">Laskutuslisä:</span>
                        <span className="font-semibold text-midnight-900">9 €/kk <span className="text-xs text-slate-500">alv 0%</span></span>
                        <span className="text-slate-600 text-right">{formatCurrency(9 * 1.255)}/kk <span className="text-xs">sis. alv</span></span>
                      </div>
                      {offer.residual_value && application.equipment_price && (
                        <div className="grid grid-cols-3 gap-2">
                          ...
                          <span className="font-semibold text-midnight-900">
                            {((offer.residual_value / application.equipment_price) * 100).toFixed(1)}% ({formatCurrency(offer.residual_value)})
                          </span>
                          <span className="text-slate-600 text-right">{formatCurrency(offer.residual_value * 1.255)} <span className="text-xs">sis. alv</span></span>
                        </div>
                      )}
                    </div>
                    <p className="text-xs text-slate-500 mt-3 italic">ALV 25,5 %</p>
                  </div>
                  ...
                  {offer.status === 'SENT' && (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4 mt-4">
                      <p className="text-green-700">
                        ✅ Tarjous lähetetty asiakkaalle. Odottaa asiakkaan hyväksyntää.
                      </p>
                    </div>
                  )}
                </div>
        ...
        {activeTab === 'contract' && (
          <div className="space-y-4">
            {/* Create contract button - available when offer exists */}
            {offerList.length > 0 && !contractList.length && !showContractForm && (
              <button
            ...
            {contractList.length === 0 && !showContractForm ? (
              <div className="card text-center py-12">
                ...
                <p className="text-slate-500">
                  {offerList.length > 0
                    ? 'Luo sopimus tarjoukselle.'
                    : 'Sopimus voidaan luoda kun tarjous on tehty.'}
                </p>
              </div>
            ) : (
                                  ...
                                  <li key={i} className="flex items-center justify-between bg-white rounded-lg p-2 border border-blue-100">
                                    ...
                                    <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">
                                      {att.type === 'tilinpaatos' ? 'Tilinpäätös' :
                                       ...
                                       att.type === 'henkilokortti' ? 'Henkilökortti / Passi' :
                                       att.type === 'ulosottotodistus' ? 'Ulosottotodistus' :
                                       att.type}
                                    </span>
                                  </li>
                                ))}
