...
export default function SetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      if (!session) {
        toast.error('Kirjautumislinkki on vanhentunut. Lähetä uusi hakemus.');
        navigate('/');
        return;
      }

      // Tarkista onko salasana jo asetettu
      ...
      if (profile?.is_verified) {
        // Jo rekisteröitynyt - ohjaa dashboardiin
        await checkAuth();
        navigate('/dashboard');
        return;
      }
        ...
        if (!existingProfile) {
          ...
          const { error: profileError } = await supabase
            ...
            .insert({
              ...
              email: user.email,
              role: 'CUSTOMER',
              first_name: metadata.first_name || metadata.company_name?.split(' ')[0] || '',
